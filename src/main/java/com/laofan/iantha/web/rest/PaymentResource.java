...
/**
 * REST controller for managing {@link com.laofan.iantha.domain.Payment}.
 */
@RestController
        ...
        Optional<Payment> result = paymentRepository
            ...
            .map(existingPayment -> {
                ...
                if (payment.getRefId() != null) {
                    existingPayment.setRefId(payment.getRefId());
                }
                if (payment.getCardPan() != null) {
                    existingPayment.setCardPan(payment.getCardPan());
                }
                if (payment.getCardHash() != null) {
                    existingPayment.setCardHash(payment.getCardHash());
                }
                if (payment.getFee() != null) {
                    existingPayment.setFee(payment.getFee());
                }
                if (payment.getIsSuccessful() != null) {
                    existingPayment.setIsSuccessful(payment.getIsSuccessful());
                }
                if (payment.getPayable() != null) {
                    existingPayment.setPayable(payment.getPayable());
                }
