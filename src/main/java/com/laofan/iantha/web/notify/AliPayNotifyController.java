...
import com.laofan.iantha.web.notify.constant.AliPayConstant;
...
public class AliPayNotifyController {
    ...
    public String notifyAliPay(@PathVariable("configId") String configId, HttpServletRequest request) throws FileNotFoundException {
        ...
        // 获取支付配置信息
        PayConfig payConfig = null; //payConfigService.selectPayConfigById(configId);
        if (payConfig == null) {
        ...
        if (flag) {
            ...
            String buyerLogonId = new String(
                request.getParameter("buyer_logon_id").getBytes(StandardCharsets.ISO_8859_1),
                StandardCharsets.UTF_8
            );
            // 公用回传参数
            String passBackParams = new String(
                request.getParameter("passback_params").getBytes(StandardCharsets.ISO_8859_1),
                StandardCharsets.UTF_8
            );
